...
import { finalizeAiDraftSchema } from '@/lib/utils/validation';
...
export async function POST(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const validated = finalizeAiDraftSchema.safeParse(body);
  if (!validated.success) {
    return errorResponse('VALIDATION_ERROR', 400, { issues: validated.error.issues });
  }

  try {
    const study = await AIDraftService.finalizeDraft(id, validated.data, {
      user: auth.user,
      company: auth.company,
    });
    return successResponse({ study }, 'Study created from AI draft', 201);
  } catch (error) {
    ...
    logger.error('POST /api/studies/ai-drafts/[id]/finalize failed', {
      id,
