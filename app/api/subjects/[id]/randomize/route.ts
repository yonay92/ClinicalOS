...
import { randomizeSubjectSchema } from '@/lib/utils/validation';
...
export async function POST(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const validated = randomizeSubjectSchema.safeParse(body);
  if (!validated.success) {
    return errorResponse('VALIDATION_ERROR', 400, { issues: validated.error.issues });
  }

  try {
    const subject = await SubjectService.randomize(id, validated.data, {
      user: auth.user,
      company: auth.company,
    });
    return successResponse(subject, 'Subject randomized');
  } catch (error) {
    ...
    logger.error('POST /api/subjects/[id]/randomize failed', {
      id,
