...
import { completeVisitSchema } from '@/lib/utils/validation';
...
export async function POST(
  ...
  const validated = completeVisitSchema.safeParse(body);
  if (!validated.success) {
    return errorResponse('VALIDATION_ERROR', 400, { issues: validated.error.issues });
  }

  try {
    const visit = await SubjectService.completeVisit(id, visitId, validated.data, {
      user: auth.user,
      company: auth.company,
    });
    return successResponse(visit, 'Visit completed');
  } catch (error) {
    ...
    logger.error('POST /api/subjects/[id]/visits/[visitId]/complete failed', {
      id,
      visitId,
