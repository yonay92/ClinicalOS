...
export async function POST(
  ...
  try {
    const visit = await VisitService.startVisit(id, visitId, {
      user: auth.user,
      company: auth.company,
    });
    return successResponse(visit, 'Visit started');
  } catch (error) {
    ...
    logger.error('POST /api/subjects/[id]/visits/[visitId]/start failed', {
      id,
      visitId,
