...
import { reopenVisitSchema } from '@/lib/utils/validation';
...
export async function POST(
  ...
  const validated = reopenVisitSchema.safeParse(body);
  if (!validated.success) {
    return errorResponse('VALIDATION_ERROR', 400, { issues: validated.error.issues });
  }

  try {
    const visit = await VisitService.reopenVisit(id, visitId, validated.data, {
      user: auth.user,
      company: auth.company,
    });
    return successResponse(visit, 'Visit reopened');
  } catch (error) {
    ...
    logger.error('POST /api/subjects/[id]/visits/[visitId]/reopen failed', {
      id,
      visitId,
