...
import { logContactAttemptSchema } from '@/lib/utils/validation';
...
export async function POST(
  ...
  const validated = logContactAttemptSchema.safeParse(body);
  if (!validated.success) {
    return errorResponse('VALIDATION_ERROR', 400, { issues: validated.error.issues });
  }

  try {
    const confirmation = await AppointmentConfirmationService.logContact(
      id,
      visitId,
      validated.data,
      {
    ...
    return successResponse(confirmation, 'Contact attempt logged');
  } catch (error) {
    ...
    logger.error('POST /api/subjects/[id]/visits/[visitId]/confirmation/contact failed', {
      id,
      visitId,
