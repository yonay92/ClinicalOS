...
import { changeSubjectStatusSchema } from '@/lib/utils/validation';
...
export async function POST(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const validated = changeSubjectStatusSchema.safeParse(body);
  if (!validated.success) {
    return errorResponse('VALIDATION_ERROR', 400, { issues: validated.error.issues });
  }

  try {
    const subject = await SubjectService.updateStatus(
      id,
      validated.data.status,
      { user: auth.user, company: auth.company },
      validated.data.reason,
    );
    return successResponse(subject, 'Subject status updated');
  } catch (error) {
    ...
    logger.error('POST /api/subjects/[id]/status failed', {
      id,
