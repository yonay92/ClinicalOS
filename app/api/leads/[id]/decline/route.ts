...
import { declineLeadSchema } from '@/lib/utils/validation';
...
export async function POST(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const validated = declineLeadSchema.safeParse(body);
  if (!validated.success) {
    return errorResponse('VALIDATION_ERROR', 400, { issues: validated.error.issues });
  }

  try {
    const lead = await LeadService.decline(id, validated.data.declined_reason, {
      user: auth.user,
      company: auth.company,
    });
    return successResponse(lead, 'Lead declined');
  } catch (error) {
    ...
    logger.error('POST /api/leads/[id]/decline failed', {
      id,
