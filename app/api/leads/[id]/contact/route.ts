...
import { logLeadContactSchema } from '@/lib/utils/validation';
...
export async function POST(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const validated = logLeadContactSchema.safeParse(body);
  if (!validated.success) {
    return errorResponse('VALIDATION_ERROR', 400, { issues: validated.error.issues });
  }

  try {
    const lead = await LeadService.logContact(id, validated.data, {
      user: auth.user,
      company: auth.company,
    });
    return successResponse(lead, 'Contact attempt logged');
  } catch (error) {
    ...
    logger.error('POST /api/leads/[id]/contact failed', {
      id,
