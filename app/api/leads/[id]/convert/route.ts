...
import { convertLeadSchema } from '@/lib/utils/validation';
...
export async function POST(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const validated = convertLeadSchema.safeParse(body);
  if (!validated.success) {
    return errorResponse('VALIDATION_ERROR', 400, { issues: validated.error.issues });
  }

  try {
    const result = await LeadService.convertToSubject(id, validated.data, {
      user: auth.user,
      company: auth.company,
    });
    return successResponse(result, 'Lead converted to Subject');
  } catch (error) {
    ...
    logger.error('POST /api/leads/[id]/convert failed', {
      id,
