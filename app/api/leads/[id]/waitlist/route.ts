...
export async function POST(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    const lead = await LeadService.waitlist(id, { user: auth.user, company: auth.company });
    return successResponse(lead, 'Lead waitlisted');
  } catch (error) {
    ...
    logger.error('POST /api/leads/[id]/waitlist failed', {
      id,
